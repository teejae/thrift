 ...
 *
 */
class t_go_generator : public t_oop_generator {
 public:
  t_go_generator(
      t_program* program,
 ...
 */
string t_go_generator::as3_package() {
 ...
 */
string t_go_generator::as3_type_imports() {
  return
    string() +
 ...
 */
string t_go_generator::as3_thrift_imports() {
  return
    string() +
 ...
 */
void t_go_generator::generate_consts(std::vector<t_const*> consts) {
  if (consts.empty()) {
    return;
  }
...
void t_go_generator::print_const_value(std::ofstream& out, string name, t_type* type, t_const_value* value, bool in_static, bool defval) {
...
string t_go_generator::render_const_value(ofstream& out, string name, t_type* type, t_const_value* value) {
  type = get_true_type(type);
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_struct(t_struct* tstruct,
                                            bool is_exception) {
  // Make output file
 ...
 */
void t_go_generator::generate_as3_struct_definition(ofstream &out,
                                                       t_struct* tstruct,
                                                       bool is_exception,
                                                       bool in_class,
                                                       bool is_result) {
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_struct_reader(ofstream& out,
                                                   t_struct* tstruct) {
  out <<
...
void t_go_generator::generate_as3_validator(ofstream& out,
                                                   t_struct* tstruct){
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_struct_writer(ofstream& out,
                                                   t_struct* tstruct) {
  out <<
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_struct_result_writer(ofstream& out,
                                                          t_struct* tstruct) {
  out <<
    indent() << "public function write(oprot:TProtocol):void {" << endl;
  indent_up();
...
void t_go_generator::generate_reflection_getters(ostringstream& out, t_type* type, string field_name, string cap_name) {
...
void t_go_generator::generate_reflection_setters(ostringstream& out, t_type* type, string field_name, string cap_name) {
  indent(out) << "case " << upcase_string(field_name) << ":" << endl;
  indent_up();
...
void t_go_generator::generate_generic_field_getters_setters(std::ofstream& out, t_struct* tstruct) {
...
void t_go_generator::generate_generic_isset_method(std::ofstream& out, t_struct* tstruct){
  const vector<t_field*>& fields = tstruct->get_members();
  vector<t_field*>::const_iterator f_iter;
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_bean_boilerplate(ofstream& out,
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_struct_tostring(ofstream& out,
                                                     t_struct* tstruct, bool bindable) {
 ...
 * @param tstruct The struct definition
 */
void t_go_generator::generate_as3_meta_data_map(ofstream& out,
                                                   t_struct* tstruct) {
  const vector<t_field*>& fields = tstruct->get_members();
  vector<t_field*>::const_iterator f_iter;
 ...
 */
std::string t_go_generator::get_as3_type_string(t_type* type) {
  if (type->is_list()){
    return "TType.LIST";
  } else if (type->is_map()) {
    return "TType.MAP";
    ...
    switch (((t_base_type*)type)->get_base()) {
      ...
      default : throw std::runtime_error("Unknown thrift type \"" + type->get_name() + "\" passed to t_go_generator::get_as3_type_string!"); break; // This should never happen!
    }
  } else {
    throw std::runtime_error("Unknown thrift type \"" + type->get_name() + "\" passed to t_go_generator::get_as3_type_string!"); // This should never happen!
  }
}

void t_go_generator::generate_field_value_meta_data(std::ofstream& out, t_type* type){
  out << endl;
  indent_up();
  indent_up();
  if (type->is_struct()){
 ...
 */
void t_go_generator::generate_function_helpers(t_function* tfunction) {
 ...
 */
string t_go_generator::type_name(t_type* ttype, bool in_container, bool in_init) {
 ...
 * @param ttype The type
 */
string t_go_generator::declare_field(t_field* tfield, bool init) {
 ...
 */
std::string t_go_generator::get_cap_name(std::string name){
  name[0] = toupper(name[0]);
  return name;
}

string t_go_generator::constant_name(string name) {
 ...
 */
void t_go_generator::generate_as3_doc(ofstream &out,
                                         t_doc* tdoc) {
  if (tdoc->has_doc()) {
...
std::string t_go_generator::generate_isset_check(t_field* field) {
  return generate_isset_check(field->get_name());
}

std::string t_go_generator::generate_isset_check(std::string field_name) {
  return "is" + get_cap_name("set") + get_cap_name(field_name) + "()";
}

void t_go_generator::generate_isset_set(ofstream& out, t_field* field) {
  if (!type_can_be_null(field->get_type())) {
    indent(out) << "this.__isset_" << field->get_name() << " = true;" << endl;
  }
}

std::string t_go_generator::get_enum_class_name(t_type* type) {
  string package = "";
